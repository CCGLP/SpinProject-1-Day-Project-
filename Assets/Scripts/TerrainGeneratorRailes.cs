...
public class TerrainGeneratorRailes : MonoBehaviour {
	[SerializeField]
	...
	void Update () {
		...
		if (timerObs > timeToSpawnObstacles) {
			...
			auxG.transform.rotation = Quaternion.Euler(new Vector3(0, 0,(0) + obstacles[aux].transform.rotation.eulerAngles.z + ((360/8) *Random.Range(0,9)))); 
